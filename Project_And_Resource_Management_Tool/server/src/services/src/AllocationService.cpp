...
Allocation AllocationService::createAllocation(int managerUserId, const CreateAllocationRequest& request) {
    ...
    auto resource = resourceRepository->findById(request.userId);
    if (!resource.has_value()) {
        throw NotFoundException("Resource not found.");
    }
    if (!resource->isActive) {
        throw ValidationException("Cannot allocate an inactive resource.");
    }
    ...
    resourceRepository->setStatus(request.userId, ResourceStatus::ALLOCATED);
...
Allocation AllocationService::endAllocation(int allocationId, int managerUserId) {
    ...
    if (!resourceRepository->hasActiveAllocations(allocation->userId)) {
        resourceRepository->setStatus(allocation->userId, ResourceStatus::BENCH);
    }
