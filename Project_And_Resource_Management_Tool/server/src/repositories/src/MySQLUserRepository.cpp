...
static const std::string USER_SELECT =
    "SELECT u_users.user_id, u_users.full_name, u_users.email, u_users.username, u_users.password_hash, "
    "r_roles.role_name AS role, u_users.is_active, u_users.password_expires_at, u_users.created_at "
    "FROM users u_users JOIN roles r_roles ON r_roles.role_id = u_users.role_id ";
    ...
    try {
        ...
        std::unique_ptr<sql::PreparedStatement> statement(
            connection->prepareStatement(
                USER_SELECT + "WHERE BINARY u_users.username = ?"
            )
        );
...
std::optional<User> MySQLUserRepository::findById(int userId) {
    try {
        ...
        std::unique_ptr<sql::PreparedStatement> statement(
            connection->prepareStatement(
                USER_SELECT + "WHERE u_users.user_id = ?"
            )
        );
...
std::vector<User> MySQLUserRepository::findAll() {
    try {
        ...
        std::unique_ptr<sql::PreparedStatement> statement(
            connection->prepareStatement(
                USER_SELECT + "ORDER BY u_users.user_id"
            )
        );
        std::unique_ptr<sql::ResultSet> resultSet(statement->executeQuery());
...
void MySQLUserRepository::updateRole(int userId, const std::string& newRole) {
    try {
        auto connection = DatabasePool::getInstance().acquire();
        std::unique_ptr<sql::PreparedStatement> statement(
            connection->prepareStatement(
                "UPDATE users SET role_id = (SELECT role_id FROM roles WHERE role_name = ?) "
                "WHERE user_id = ?"
            )
        );
        ...
        throw AppException(std::string("Database error in updateRole: ") + sqlException.what());
    }
}

int MySQLUserRepository::countActiveAdmins() {
    try {
        auto connection = DatabasePool::getInstance().acquire();
        std::unique_ptr<sql::PreparedStatement> statement(
            connection->prepareStatement(
                "SELECT COUNT(*) FROM users u_users "
                "JOIN roles r_roles ON r_roles.role_id = u_users.role_id "
                "WHERE r_roles.role_name = 'ADMIN' AND u_users.is_active = 1"
            )
        );
        std::unique_ptr<sql::ResultSet> resultSet(statement->executeQuery());
        resultSet->next();
        ...
        throw AppException(std::string("Database error in countActiveAdmins: ") + sqlException.what());
    }
}
