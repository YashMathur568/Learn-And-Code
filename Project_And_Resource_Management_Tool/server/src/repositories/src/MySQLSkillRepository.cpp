...
ResourceSkill MySQLSkillRepository::mapRowToSkill(sql::ResultSet* resultSet) {
    ResourceSkill skill;
...
std::vector<ResourceSkill> MySQLSkillRepository::findByUserId(int userId) {
    try {
...
std::optional<ResourceSkill> MySQLSkillRepository::findById(int skillId) {
    try {
        auto connection = DatabasePool::getInstance().acquire();
        std::unique_ptr<sql::PreparedStatement> statement(
            connection->prepareStatement(
...
int MySQLSkillRepository::create(const ResourceSkill& skill) {
    try {
        auto connection = DatabasePool::getInstance().acquire();
        std::unique_ptr<sql::PreparedStatement> statement(
            connection->prepareStatement(
...
void MySQLSkillRepository::update(const ResourceSkill& skill) {
    try {
        auto connection = DatabasePool::getInstance().acquire();
        std::unique_ptr<sql::PreparedStatement> statement(
            connection->prepareStatement(
