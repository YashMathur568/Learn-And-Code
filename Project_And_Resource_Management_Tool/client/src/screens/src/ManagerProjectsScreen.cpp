...
static void showProjectDetail(const ApiClient& api, const nlohmann::json& proj) {
    while (true) {
        ...
        const auto& milestones = proj.value("milestones", nlohmann::json::array());
        if (!milestones.empty()) {
            std::cout << "\nMilestones:\n";
        ...
        if (!allocs.empty()) {
            ...
            for (const auto& allocation : allocs) {
                ...
                std::cout << ConsoleUtil::col(allocation.value("resourceName", ""), 22)
        ...
        if (selectedOption == "allocation" || selectedOption == "A") {
            ...
            ConsoleUtil::printError("Invalid option. Press A for AI risk summary or B to go back.");
            ConsoleUtil::pause();
        }
    }
}
...
void showManagerProjects(const ApiClient& api) {
    while (true) {
        ...
        int sel = 0;
        try { sel = std::stoi(selectedOption) - 1; }
        catch (...) {
            ConsoleUtil::printError("Please enter a valid project number.");
            ConsoleUtil::pause();
            continue;
        }
        if (sel < 0 || sel >= static_cast<int>(projects.size())) {
            ConsoleUtil::printError("Number out of range. Please select from the list.");
            ConsoleUtil::pause();
            continue;
        }
